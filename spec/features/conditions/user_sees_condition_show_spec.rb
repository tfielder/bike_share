...
describe 'as a visitor' do
  describe 'when visiting /condition/2 show page' do
    it 'shows unique conditions for show page' do
      ...
      expect(page).to_not have_content("Date: #{condition_1.date.strftime("%m/%d/%Y")}")
      expect(page).to_not have_content("Max Temperature: #{condition_1.max_temp}")
      expect(page).to_not have_content("Mean Temperature: #{condition_1.mean_temp}")
      expect(page).to_not have_content("Min Temperature: #{condition_1.min_temp}")
      expect(page).to_not have_content("Mean Humidity: #{condition_1.mean_humidity}")
      expect(page).to_not have_content("Mean Visibility: #{condition_1.mean_visibility}")
      expect(page).to_not have_content("Mean Wind Speed: #{condition_1.mean_wind_speed}")
      expect(page).to_not have_content("Precipitation: #{condition_1.precip}")
